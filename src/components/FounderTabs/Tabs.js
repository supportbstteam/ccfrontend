    ...
    useEffect(() => {
      async function fetchDataFromAPI() {
        try {
          ...
          setData(upData);
          setLoading(false);
          console.log(upData); // Use upData.id instead of data.id
        } catch (error) {
    ...
    return (
        <div className='container'>
            <div className='tabs founders-tabs' data-aos="fade-in">
                ...
                {data.map((tab, i) =>
            <img key={i} src={`https://teamwebdevelopers.com/charge_construct/public/images/team/`+tab.image}  id={i}  onClick={(handleTabClick)}/>
                )}
            </div>
